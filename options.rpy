...
define config.name = _("SpornbobBETA")
...
init python:
    ...
    build.archive("game", "all")  # Cria um arquivo game.rpa para esconder os assets

    build.classify("game/**.png", "game")     # Imagens
    build.classify("game/**.jpg", "game")
    build.classify("game/**.webp", "game")

    build.classify("game/**.mp3", "game")     # Áudio
    build.classify("game/**.ogg", "game")
    build.classify("game/**.wav", "game")

    build.classify("game/**.rpy", None)       # Remove .rpy do build
    build.classify("game/**.psd", None)       # Arquivos editáveis também

    build.classify("**~", None)               # Arquivos temporários
